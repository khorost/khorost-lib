...
namespace khorost {
    namespace network {
        ...
        public:
            ...
            virtual void get_expire_shift(boost::posix_time::time_duration& value) {
                value = boost::posix_time::minutes(DEFAULT_EXPIRE_MINUTES);
            }
        ...
        class session_controller final {
            ...
            public:
                ...
                bool load_sessions(session_controller& session_controler,
                                   const std::function<session_ptr(const std::string& session_id, boost::posix_time::ptime created,
        ...
        public:
            ...
            bool open(const std::string& driver, int version_min, int version_current,
                      const std::function<session_ptr(const std::string& session_id, const boost::posix_time::ptime& created,
                                                      const boost::posix_time::ptime& expired)> creator);

            session_ptr find_session(const std::string& session_id);
            session_ptr get_session(const std::string& session_id, bool& created,
                                    std::function<session_ptr(const std::string& session_id, const boost::posix_time::ptime& created,
                                                              const boost::posix_time::ptime& expired)> creator);
